...
def write_results_csvs(
    ...
    df_stages.set_index("stage_name", inplace=True)

    df_boiler = df_summary[df_summary["stage_name"] == "TOTAL_BOILER"].copy()

    # Global water mass flow
    try:
        m_w = global_profile.water[0].mass_flow.to("kg/s").magnitude
    except Exception:
        m_w = float("nan")

    # Total boiler steam capacity [t/h]
    boiler_steam_tph = m_w * 3600.0 / 1000.0  # kg/s → t/h

    # Per-stage enthalpy rise [kJ/kg]
    if "water_out_h[kJ/kg]" in df_stages.columns and "water_in_h[kJ/kg]" in df_stages.columns:
        dh_stage = df_stages["water_out_h[kJ/kg]"] - df_stages["water_in_h[kJ/kg]"]
        dh_total = dh_stage.sum()

        if dh_total != 0:
            frac = dh_stage / dh_total
        else:
            frac = 0.0

        # Steam generated by each stage [t/h]
        df_stages["steam_generated[t/h]"] = boiler_steam_tph * frac
    else:
        # Fallback: no enthalpy data, set NaN
        df_stages["steam_generated[t/h]"] = float("nan")

    df_stages.to_csv(stages_summary_path)
    ...
    if not df_boiler.empty:
        ...
        # Drop identity columns not wanted in boiler summary
        for col in ["stage_index", "stage_name", "stage_kind"]:
            if col in boiler_df.columns:
                boiler_df.drop(columns=[col], inplace=True)
        ...
        # Reshape to two-column table: variable, value
        boiler_tidy = boiler_df.melt(var_name="variable", value_name="value")

        # Optionally drop empty rows (NaN or "")
        boiler_tidy = boiler_tidy[~(boiler_tidy["value"].isna() | (boiler_tidy["value"] == ""))]

        # Write with no index
        boiler_tidy.to_csv(boiler_summary_path, index=False)
